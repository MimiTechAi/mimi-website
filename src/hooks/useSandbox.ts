...
export function useSandbox(): [SandboxState, SandboxActions] {
    ...
    useEffect(() => {
        const initServices = async () => {
            try {
                ...
                // Initialize Package Manager (lazy load list)
    ...
    const addTerminalLine = useCallback((text: string, type: TerminalLine['type']) => {
        ...
        setState(prev => ({
            ...prev,
            terminalLines: [...prev.terminalLines.slice(-100), line], // Keep last 100 lines
            isOpen: true, // Auto-open when terminal activity
        }));
    }, []);
    ...
    // useMemo to stabilize actions object - prevents infinite loops
    // when used in useEffect dependencies
