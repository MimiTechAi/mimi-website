...
export const ChatPanel = memo(function ChatPanel() {
    ...
    return (
        <ErrorBoundary fallbackTitle="UI Absturz abgefangen" fallbackMessage="Ein Darstellungsfehler im Chat wurde isoliert.">
            <div className="mimi-panel panel-center">
                ...
                {/* Status Header Area */}
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: '100%', padding: '0 20px', gap: '8px', zIndex: 10 }}>
                    ...
                    {/* Privacy Badge — moved below pill to not interrupt the massive glass header */}
                    <div
                ...
                {/* Virtual Computer toggle */}
                <button
                    className="sandbox-toggle-btn"
                    onClick={() => ctx.setVirtualComputerOpen((prev: boolean) => !prev)}
                    title={ctx.isVirtualComputerOpen ? 'Workspace ausblenden' : 'Workspace einblenden'}
                    aria-label={ctx.isVirtualComputerOpen ? 'Workspace ausblenden' : 'Workspace einblenden'}
                >
                    {ctx.isVirtualComputerOpen ? '▷' : '💻'}
                </button>
                    ...
                    <div className="chat-messages" role="log" aria-live="polite" aria-atomic="false">
                                ...
                                {msg.role === "user" ? (
                                    ...
                                    <div className="msg-agent">
                                        ...
                                        <div className="agent-body">
                                            {/* Collapsible Action Blocks (Manus-style) */}
                                                ...
                                                return (
                                                    <div className="action-blocks-container">
                                                        {myPills.map(pill => (
                                                            <details
                                                                key={pill.id}
                                                                className={`action-block action-block--${pill.status}`}
                                                            >
                                                                <summary className="action-block-summary">
                                                                    <span className={`action-block-icon action-block-icon--${pill.status}`}>
                                                                        {pill.status === 'running' ? '⟳' :
                                                                            pill.status === 'done' ? '✓' : '✗'}
                                                                    </span>
                                                                    <span className="action-block-label">
                                                                        {TOOL_LABELS[pill.toolName] || pill.toolName}
                                                                    </span>
                                                                    {pill.duration && (
                                                                        <span className="action-block-duration">
                                                                            {(pill.duration / 1000).toFixed(1)}s
                                                                        </span>
                                                                    )}
                                                                    <span className="action-block-arrow">▸</span>
                                                                </summary>
                                                                {pill.output && (
                                                                    <div className="action-block-output">
                                                                        <pre>{pill.output.slice(0, 500)}</pre>
                                                                    </div>
                                                                )}
                                                            </details>
                                                        ))}
                                            ...
                                            {hoveredMsgIdx === idx && (
                                                <div className="msg-actions">
                                                    <button onClick={() => ctx.handleCopyMessage(msg.content)} title="Kopieren"><Copy className="w-3.5 h-3.5" /></button>
                                                    <button onClick={() => { if (typeof navigator !== 'undefined' && navigator.vibrate) navigator.vibrate(50); ctx.handleRetryMessage(idx); }} title="Nochmal generieren"><RefreshCw className="w-3.5 h-3.5" /></button>
                                                </div>
                                            )}
                                        </div>
                                    </div>
                        ...
                        {ctx.isGenerating && !ctx.currentResponse && typingTimedOut && (
                            <div className="msg-agent">
                                ...
                                <div className="agent-body">
                                    <div className="agent-bubble">
                                        <div className="agent-text" role="alert" style={{ opacity: 0.8, fontSize: '0.85rem', lineHeight: 1.5 }}>
